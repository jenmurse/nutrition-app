...
export default function RecipeDetailPage() {
  ...
  return (
    <div className="h-full relative">
      ...
      {/* ── Main Scroll ── */}
      <div id="rd-scroll-container" className={`h-full overflow-y-auto${recipe.image ? " rd-has-image" : ""}`}>

<div className="detail-content max-w-[1100px] mx-auto animate-page-enter" style={{ padding: "0 64px 120px 196px" }}>
                ...
                {recipe.instructions.split("\n").filter(s => s.trim()).map((step, idx) => {
                  ...
                  return (
                    <div key={idx} className={`flex gap-4 items-baseline py-5 ${idx < steps.length - 1 ? "border-b border-[var(--rule)]" : ""}`}>
                      ...
                      <span className="text-[13px] leading-[1.7] text-[var(--fg-2)]">{step.replace(/^\d+[\.\)]\s*/, "")}</span>
                    </div>
                  );
                })}
              </div>

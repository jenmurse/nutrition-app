...
**2. Native app — B2 plan (Capacitor + Supabase backend)** — `briefs/going-native-b2-plan.md`
DUNS resolved + Apple Developer account approved (June 16, 2026) — **trigger met, ready to build.** Wrap the existing Next.js app in Capacitor (iOS first, Android fast-follow; phone + tablet), **consolidate the backend onto Supabase Pro and retire Railway**, host landing/privacy on Vercel, recipe images → Supabase Storage, MCP becomes a hosted remote connector (talks to Supabase cloud, not the device). Adds "Sign in with Apple" alongside Google. Privacy policy expanded for store submission (done June 16). **RLS** (currently deferred) becomes the main backend task. **Monetization decided** (June 16, 2026) — see `briefs/monetization-decision.md`. Two tiers: Free (single person, manual planning, ~5 optimizer runs) + Pro ($7/mo or $60/yr: household, unlimited optimizer, day templates, MCP). No ads. Positioning also decided — see `briefs/positioning-social-decision.md`. On-ramp: Variant A (keep API on Vercel serverless, fast) → Variant B (pure Supabase, host only landing). Local-first rejected. Full plan + cost analysis in the brief.

...
**2. Native app — Capacitor (iOS → TestFlight → Android)** — `briefs/capacitor-build-handoff.md` (execution) + `briefs/going-native-b2-plan.md` (strategy)
**Backend phases COMPLETE (June 19–20).** Migrated to Supabase Pro, app live on Vercel serverless (Variant A), Railway + R2 retired, MCP remote connector built, Apple Sign In configured (provider side), email live. **Remaining = the native shell:** scaffold Capacitor, wire native plugins, iPad layout pass, TestFlight, then Android. Key architecture decision: the app is SSR + API routes + cookie auth, so **Capacitor wraps the live site via `server.url`** (no static export, no frontend rewrite) — the app is already a polished PWA (manifest, SW, safe areas). Main risk is Apple Guideline 4.2 (web-wrapper rejection), mitigated by real native value (Sign in with Apple, keychain, haptics, status bar, share, local notifications, offline). Open decisions to lock at session start: bundle ID (`com.withgoodmeasure.app`), reserve "Good Measure" in App Store Connect, deep-link `goodmeasure://`. Machine prereqs: install CocoaPods, repoint `xcode-select` to full Xcode. **Monetization decided** (`briefs/monetization-decision.md`): Free + Pro ($7/mo·$60/yr, MCP gated) — built but not yet UI-enforced. See the handoff brief for the full step-by-step.
...
- **Landing v6** — replaced chapter-based landing with scenario-driven layout. Route group `app/(marketing)/`, scoped CSS `app/(marketing)/landing.css` (all `ln-*` prefixed), 9 components (`Topbar`, `Hero`, `ImageBand`, `Interstitial`, `Scenario`, `Architecture`, `Close`, `Footer` — `Callout` unused). Three scenarios with sticky stages + beat structure. Architecture copy: "Everything you cook. Connected to your AI." Type system: three display tiers (hero h1 only / section h2s / sub h3s), all weight 500, tracking tightened by tier (-0.04em / -0.03em / -0.02em).

**Landing copy revised (June 15, 2026 — native-first rebalance, after chat UI was hidden).** The original page leaned on in-app AI chat (Scn 01/02 "chat with your AI"), which no longer exists in-app. Rewrote to lead with native features and reposition AI as the optional Claude Desktop / MCP layer:
- **Scn 01 (recipe dial-in)** — now leads with native live-nutrition editing; the connected assistant is the optional "talk it through" path. Removed the in-app chat placeholder.
- **Scn 02** — converted from "blank week → AI fills it" to the **meal optimizer** ("A day that misses your targets. Fix it by the numbers."): off-target day → pick up to 3 targets → three variations → apply. Emphasizes "no AI, no cost, just math." New placeholder `PhOptResults` (three-variation card row); removed now-unused `PhChat` / `PhFilledWeek`.
- **Scn 03 (templates)** — softened the AI line; added the shopping-list beat (E) as the closing payoff (moved out of old Scn 02).
- **Architecture** — repositioned: native does the daily work alone; connect your assistant in Claude Desktop for open-ended work (propose a week, author a recipe, talk through edits). "Bring your own assistant, no separate subscription."
- **Hero + meta** — "plans optimized with AI" → "a planner that optimizes your days to hit your targets — by math, not guesswork."
Screenshot slots re-labeled to match the new Scn 02 (optimizer picker / three variations / applied day).
...
- **Food photos shipped (June 16, 2026)** — all 8 food photo slots filled: image band (Breakfast eggs, Lunch poke, Dinner rigatoni, Dessert lemon bars) + both interstitials (Snack apple, Dessert cookies, Breakfast oats, Dinner noodle-shrimp). `ImageBand` and `Interstitial` render photo-only when `src` is set (no overlay label). Photos optimized via `sips` (quality 80, ≤1000px, JPG) in `public/landing/`. Scn 01 recipe renamed Miso-Glazed Tofu (was Sesame Miso Cannellini Beans); recipe hero photo slot wired in desktop + mobile `RecipeView`. **Landing is now complete** — the sticky-scene stage visuals are 15 live React demo components (`app/(marketing)/_demo/screens/`, desktop + mobile variants), not screenshot slots. The earlier "13 app screenshots pending" note was a relic of the original v6 plan; no screenshots are needed.
...
**Shipped this session (June 15, 2026):**
- **Meal optimizer** — Math-based day nutrition optimizer. Day ⋯ → "Optimize this day". `lib/mealOptimizer.ts` (pure engine), `app/api/meal-plans/[id]/optimize/route.ts`, `app/planner/DayOptimizer.tsx`. 1–3 target nutrients, 3 labeled variations, swap/remove/limited-add lane model, favorites-first, meal-type-matched. Desktop opens full-screen surface (nested under nav, like Compare); mobile is a bottom sheet. Verified on both.
- **⋯ launcher hub** — Day overflow menu is now a hub: Optimize / Apply template / Save / Manage. Desktop: Optimize + Apply + Manage open as right-side flyouts (`mx-manage-sheet` pattern); Save opens right-side flyout with name + overwrite-existing. Mobile: Optimize pushes bottom sheet, Apply/Save/Manage are pushed sub-views inside the ⋯ sheet (same view-state pattern as Apply). All four actions use consistent `doSaveTemplate` helper for save logic.
- **In-app chat hidden** — `SHOW_CHAT = false` in `lib/featureFlags.ts`. ChatTrigger gated in TopNav + MobileTopBar; ChatPanel gated in layout.tsx. ChatProvider kept alive. All code/API/MCP intact — flip flag to restore.
...
**Native app — local-first (Yjs/CRDT sync)** — `briefs/_archived/native-app-tracks.md` (Track 2)
Real architectural shift: SQLite/IndexedDB on each device, Yjs CRDT sync through a relay server. ~35 weeks. **Considered and set aside (June 16, 2026)** in favor of the Supabase-backed B2 plan — local-first fights the household-sharing model and isn't needed (MCP talks to the cloud, not the device). Retained only as a theoretical future option. **Triggers to re-evaluate:** sustained scale where the Supabase bill becomes material, or recurring offline-write/cross-device-sync demand the cloud model can't meet.

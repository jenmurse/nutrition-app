...
DUNS resolved + Apple Developer account approved (June 16, 2026) — **trigger met, ready to build.** Wrap the existing Next.js app in Capacitor (iOS first, Android fast-follow; phone + tablet), **consolidate the backend onto Supabase Pro and retire Railway**, host landing/privacy on Vercel, recipe images → Supabase Storage, MCP becomes a hosted remote connector (talks to Supabase cloud, not the device). Adds "Sign in with Apple" alongside Google. Privacy policy expanded for store submission (done June 16). **RLS** (currently deferred) becomes the main backend task. **Monetization is an open strategic question** (free download; leaning MCP-gated/freemium; ads rejected — conflicts with no-tracking privacy stance). On-ramp: Variant A (keep API on Vercel serverless, fast) → Variant B (pure Supabase, host only landing). Local-first rejected. Full plan + cost analysis in the brief.

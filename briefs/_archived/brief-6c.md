...
3. **Hero spacing** — tighten the gap between the hero headline and body paragraph to match §02's headline-to-body spacing.
4. **§ METHOD section removed** — the standalone § METHOD section ("Cook by the gram. Plan by the week.") is deleted from the page. Its content is absorbed into the §04 headline.
5. **§04 merged + restructured** — §04 absorbs the § METHOD content as its new headline ("Cook by the gram.<br />Plan by the week."). The previous §04 headline ("If this sounds like how you want to cook...") is dropped entirely. Body is the original two-sentence autobiographical close. Layout changes from two-column to left-aligned single column matching §02 and §03.
6. **§04 attribution removed** — the mid-page `— JEN MURSE, GOOD MEASURE` line between the §04 body and GET STARTED button is removed. Page byline at the bottom carries the credit.
7. **Running bar** — five labels reduced to four with new wording.
8. **Onboarding Welcome** — new headline + body.
9. **Auth Sign in** — new headline + body.
10. **Auth Create account** — new headline + body.
11. **Italic system removed** — every `<em>` on landing, §01, §02 headline, §03 headline, and auth headlines. The `.auth-headline em` rule and Instrument Serif typeface come out of the system.
12. **Design system documentation** — typography section, em accent section, color rule section, and auth pattern section all updated to reflect the two-typeface system and the sage-as-theme-color clarification.
13. **Master plan + APP-INVENTORY** — log Step 6 close with decision log entries.
...
**Spacing fix:** The hero currently has a large gap between the headline and the body paragraph. This was tuned for the previous longer headline ("A nutrition app for people who actually cook.") that wrapped naturally across multiple lines at smaller display sizes. With the new two-line crisp headline from the manual `<br />`, that loose spacing reads as too much air. Match the headline-to-body spacing from §02 (which sits tight against its body paragraph). The exact CSS adjustment depends on what's currently there — could be a `margin-bottom` on the hero `<h1>`, a `padding-top` on the body paragraph, or a parent flex/grid `gap`. Whatever §02 uses, the hero should match.
...
```jsx
<h2>Cook by the gram.<br />Plan by the week.</h2>
```

The `<br />` between the two sentences is intentional. They're parallel imperatives that read as a directive pair — same shape, same cadence — and the line break visually separates them. Two lines, designed wrap.

This headline absorbs the content of the previous § METHOD section (see "§ METHOD removed" below). It replaces the previous §04 headline ("If this sounds like how you want to cook, I'd like you to try it.") which was demoted as part of the merge.
...
(Same two-sentence body as before. The previous "If this sounds like how you want to cook, I'd like you to try it." line is dropped from the body — with the new directive headline and the GET STARTED CTA both functioning as invitations, that opening sentence became a third stacked invitation. Three beats now have distinct jobs: headline = directive, body = autobiography + handoff, CTA = action.)

**Layout:**

The current §04 renders as a two-column layout (headline left, body and CTA right) with significant whitespace at the top of the section. Restructure §04 to a left-aligned single-column layout matching §02 and §03 — headline on top, body below, GET STARTED button below body. The two-column layout was a residue of when the section was sparser; with the merged content it has enough substance for single-column treatment.
...
The previous § METHOD section (containing only the line "Cook by the gram. Plan by the week.") is removed from the landing entirely. Its content is absorbed into the §04 headline as part of the merge described above. Delete the entire section — eyebrow, headline, container, any wrappers — from the landing JSX. The page goes from six sections to five (§00 hero, §01 positioning, §02 library, §03 matrix, §04 invitation).
...
Replace each section's copy with the locked text above. Keep the existing JSX structure (eyebrows, headline tags, body paragraphs) where it still applies; only the inner text changes for §01, §02, §03. Remove any `<em>` tags within these sections.

**The hero (§00) specifically:**

The existing headline `<h1>` likely contains something like `for people who <em>actually</em> cook`. Replace the entire `<h1>` content with `Measure<br />what matters.` — note the manual `<br />` is intentional and required (see §00 locked copy above for rationale). No inner spans or italic tags.

Tighten the headline-to-body spacing in §00 to match §02's tighter headline-to-body relationship. See §00 locked copy for the spacing fix detail.

**The § METHOD section specifically:**

The current page has a standalone § METHOD section between §03 and §04 containing only the headline "Cook by the gram. Plan by the week." Delete this entire section from the JSX — eyebrow, headline, container, and any associated wrappers. The page goes from six sections to five.

**The §04 specifically:**

§04 needs three changes: new headline, restructured layout, and attribution removal.

1. Replace the §04 headline (currently "If this sounds like how you want to cook, I'd like you to try it.") with `Cook by the gram.<br />Plan by the week.` — using a manual `<br />` between the two sentences. This headline is moved over from the deleted § METHOD section.

2. Restructure the §04 layout from two-column (headline-left, body-right) to left-aligned single-column matching §02 and §03 — headline on top, body below, GET STARTED button below body. The grid/flex container that creates the two-column layout should be replaced or rewritten to a single-column flow.

3. Remove the `— JEN MURSE, GOOD MEASURE` attribution line that currently renders between the body paragraph and the GET STARTED button. Delete the element and any wrapping container or class.

Keep the body copy as the existing two-sentence autobiographical close — only the headline changes.
...
- Add a "Locked this session (May 2 — Step 6)" subsection under "Design decisions locked" with these entries:
  ...
  - **§ METHOD section removed from landing.** Its headline absorbed into §04 as "Cook by the gram. Plan by the week." Page goes from six sections to five.
  - **§04 restructured to single-column layout.** Matches §02 and §03 pattern (headline on top, body below, CTA below body, all left-aligned). The previous two-column layout was a residue of when the section was sparser; with the merged content from § METHOD, single-column treatment is the right fit.
...
3. **Verify the hero spacing is tight.** The body paragraph should sit close to the headline, matching the headline-to-body gap used in §02. If there's a large empty gap between "what matters." and "For years I ran my household...", the spacing fix wasn't applied.
4. **Verify § METHOD section is gone.** Scrolling from §03 (matrix) directly to §04 (invitation) — no intermediate section with "Cook by the gram. Plan by the week." as standalone content. If § METHOD is still rendering as its own section, deletion wasn't complete.
5. **Verify §04 has the new merged headline.** §04's headline should be "Cook by the gram." on line 1 and "Plan by the week." on line 2 (with `<br />` between). The previous §04 headline ("If this sounds like how you want to cook...") should not appear anywhere on the page.
6. **Verify §04 layout is single-column.** Headline on top, body below, GET STARTED button below body. All left-aligned. If the page still renders §04 as two columns (headline left, body right), the layout restructure wasn't applied.
7. **Verify §04 has no mid-page attribution.** Between the body paragraph "Now it's here for you to try." and the GET STARTED button, there should be no signature, byline, or attribution line. The page byline at the bottom (`© 2026 · MADE BY JEN MURSE`) is the only credit on the page.
8. **Run STOP-CHECK Gate 1** (italic removal greps). Zero matches in target surfaces.
9. **Run STOP-CHECK Gate 2** (Instrument Serif removal greps). Zero matches anywhere.
10. **Read the onboarding Welcome screen.** Headline matches landing exactly. Body line is the new locked text.
11. **Read both auth tabs (Sign in and Create account).** Headlines and bodies match locked text. No `<em>` in either.
12. **Render check.** The landing should look visually identical to the design intent — five sections (hero, positioning, library, matrix, invitation), tight hero spacing, single-column §04, no mid-page attribution, no italic anywhere.
13. **Dashboard greeting still shows the person's name in theme-accent color.** This is the one `<em>` that survives — confirm it didn't get accidentally swept.

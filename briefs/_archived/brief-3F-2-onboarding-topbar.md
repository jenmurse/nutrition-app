# BRIEF 3F-2 — Onboarding topbar simplification (revised)
...
**Blocks:** Nothing.
**Supersedes:** brief-3F-onboarding-topbar.md (original draft, written before the eyebrow / letter-spacing / wordmark-size locks landed).
...
This brief reconciles with several locks that landed AFTER the original 3F draft was written. If you've read the earlier draft, these are the deltas:

1. **Mono labels in the topbar locked to 9px** (was 11px). This applies to `.ob-step-counter` (`STEP · 01 / 03`, `WELCOME`, `READY`).
2. **Letter-spacing locked to 0.14em** (was 0.16em).
3. **Topbar wordmark size locked to 13px to match main app nav** (was 18px). This is also being applied to the auth topbar in 3E-2 for cross-surface consistency.
4. **Topbar height uses `var(--nav-h)` token** (was hardcoded 50px).
...
**Wordmark at 13px to match main app nav.** The original brief draft had 18px; that was a stale spec. The main app's top nav uses 13px DM Sans 700 letter-spacing -0.02em, and the auth topbar (per 3E-2) uses the same. The onboarding topbar should match. When wordmark integration (3C) lands, the locked `<BrandName />` component swaps in.

**Step counter at 9px / 0.14em** — matches the system-wide eyebrow token. The step counter is a page-level editorial marker (same role as `§ NUTRITION COMPARISON` on Compare or `§ APR 26 - MAY 2` on Shopping), just positioned in the topbar's right slot rather than above a headline. Same role, same token.
...
**Note on rendering at 9px.** The middle-dot at 9px DM Mono with 0.14em letter-spacing should still read clearly. If during review the dot looks crowded or too small to register as a separator, the implementer can swap to a wider spacing variant (e.g., `· ` with a regular space on either side instead of just letter-spacing) — but only after visual verification that 9px is genuinely insufficient. Default to the spec.
...
- `§ WELCOME` / `§ READY` eyebrow stays (uses the system-wide 9px eyebrow token)
...
- `§ YOUR PROFILE` / `§ YOUR HOUSEHOLD` / `§ DAILY GOALS` eyebrow stays (9px)
...
Wordmark and step counter sizes stay the same on mobile (13px / 9px) — they're already small enough to fit comfortably at 375px. No size step-down needed. The body content's responsive behavior is unchanged.
...
- [ ] Onboarding layout wrapper has `data-register="editorial"`. In DevTools, computed value of `--bg` resolves to `#F5F4EF` (cream), not white.
- [ ] All values in `.ob-*` rules use tokens. No hardcoded `#FFFFFF`, no hardcoded `white`.
...
- [ ] `STEP · 01 / 03` doesn't wrap or truncate.
- [ ] Body content responsive behavior unchanged.

### DevTools

- [ ] Inspect the topbar. Computed styles show `display: flex` with `justify-content: space-between`. Height = `var(--nav-h)` (50px) on desktop and mobile.
- [ ] Inspect the wordmark. Computed `font-size` is 13px, `font-weight` 700, `letter-spacing` -0.02em.
- [ ] Inspect the step counter. Computed `font-size` is 9px, `font-family` is DM Mono, `letter-spacing` is 0.14em, `text-transform` is uppercase, `color` resolves to `var(--muted)`.
...
- `font-size: 11px` on `.ob-step-counter` — should not appear (locked at 9px)
- `letter-spacing: 0.16em` on any onboarding chrome — should not appear (locked at 0.14em)
- `font-size: 18px` on `.wm` in onboarding scope — should not appear (locked at 13px)
- Hardcoded `height: 50px` on `.ob-topbar` — should use `var(--nav-h)` instead

### Cross-surface consistency check

- [ ] Open onboarding Step 2 and `/auth/sign-in` in adjacent tabs.
- [ ] Topbar wordmark on both should be visually identical: 13px DM Sans 700 letter-spacing -0.02em.
- [ ] Topbar height on both should be identical (`var(--nav-h)`).
- [ ] Topbar background and hairline below should be identical (cream + 1px `var(--rule)` rule below).
- [ ] If they differ, one is wrong. Fix to match.

- [ ] Open onboarding Step 2 and `/recipes/[some-recipe-id]`.
- [ ] The step counter `STEP · 01 / 03` and the eyebrow on recipe detail (`DESSERT`) should both be 9px DM Mono uppercase 0.14em with `var(--muted)` color. Same token, same role.

## Out of scope

- **Wordmark integration.** "Good Measure" Title Case 13px in the topbar stays for now. Locked wordmark swap is deferred (3C).
...
- This is a small brief. The change is one editorial element removal plus a wordmark addition to the topbar, plus the size/letter-spacing reconciliation. Should be a short PR.
- The reason `§ ONBOARDING` was redundant: the user is going through a one-time flow with their own URL prefix and a step counter. They don't need a label confirming they're in onboarding.
- The topbar pattern (wordmark left + small mono right) is reusable. Auth uses it (wordmark + BACK), now onboarding uses it (wordmark + step counter). Consider promoting this to a shared `<EditorialTopbar>` component if the auth and onboarding layouts share enough structure to justify it. Not required for this brief, but worth noting since this is the second surface to use the pattern.
- The body wordmark on Welcome and Complete bookends stays at 32px. This is correct and intentional — the bookend pages are ceremonial moments where the brand is meant to register at full size. The topbar wordmark at 13px is the chrome-level identifier; the body wordmark at 32px is the ceremonial moment. Don't merge the two.
- The Welcome page currently has the body wordmark *and* a `§ WELCOME` body eyebrow *and* the topbar `WELCOME` label. After this brief: topbar `WELCOME` + body wordmark + body `§ WELCOME` eyebrow. The two `WELCOME`s in different positions (topbar mono small, body eyebrow mono small with `§`) are intentional — they reinforce each other rather than compete. Verify during review that this doesn't feel redundant; if it does, the topbar `WELCOME` could be replaced with empty space or with `STEP · 00 / 03`-style markup, but the current spec keeps `WELCOME` for symmetry with `READY` on Complete.
- At 9px the step counter is small. Verify that `STEP · 01 / 03` reads clearly on the actual rendered surface. If the middle dot looks ambiguous at this size, that's a visual finding worth flagging — but the size itself is locked, so the fix would be in spacing or character choice, not in bumping the size back up.

## Doc updates after code lands

- `design-system.md` — once the editorial topbar pattern is used by 3 surfaces (auth, onboarding, and a third whenever it appears), promote to a documented shared pattern. Note the wordmark-left + small-mono-right convention.
- `master-plan.md` — log Brief 3F-2 close date.

...
1. **Landing copy** — full rewrite across §00–§04 (locked text below). Zero em-dashes. New hero headline.
2. **Running bar** — five labels reduced to four with new wording.
3. **Onboarding Welcome** — new headline + body.
4. **Auth Sign in** — new headline + body.
5. **Auth Create account** — new headline + body.
6. **Italic system removed** — every `<em>` on landing, §01, §02 headline, §03 headline, and auth headlines. The `.auth-headline em` rule and Instrument Serif typeface come out of the system.
7. **Design system documentation** — typography section, em accent section, and auth pattern section all updated to reflect the two-typeface system.
8. **Master plan + APP-INVENTORY** — log Step 6 close with decision log entries.
...
Expected matches: **zero in copy-related rules.** The only italic CSS allowed in `globals.css` after this brief is whatever the dashboard greeting needs (sage accent — and that uses `font-style: normal` because the sage convention is color, not italic).
...
The hero specifically: the existing headline `<h1>` likely contains something like `for people who <em>actually</em> cook`. Replace the entire `<h1>` content with `Measure what matters.` — no inner spans or tags.
...
Currently describes two `<em>` conventions (sage accent and serif italic). Update to describe only the theme-accent convention, with corrected framing (sage is not a brand color):
...
2. **Run STOP-CHECK Gate 1** (italic removal greps). Zero matches in target surfaces.
3. **Run STOP-CHECK Gate 2** (Instrument Serif removal greps). Zero matches anywhere.
4. **Read the onboarding Welcome screen.** Headline matches landing exactly. Body line is the new locked text.
5. **Read both auth tabs (Sign in and Create account).** Headlines and bodies match locked text. No `<em>` in either.
6. **Render check.** The landing should look visually identical except for the copy itself. Layout, type sizing, eyebrows, sections — all unchanged. If anything visual shifted, something else broke.
7. **Dashboard greeting still shows the person's name in sage accent color.** This is the one `<em>` that survives — confirm it didn't get accidentally swept.
...
- Does not touch the dashboard greeting `<em>{personName}</em>` (sage accent convention; survives).
